...
CLUSTER_NAME = 'insight-data-pipeline'
REGION = 'us-central1'
PROJECT_ID = 'datapipeline01-377016'
CODE_BUCKET_NAME = 'data-pipeline-insight-combustiveis-brasil-pyspark-code'
...
with DAG(
    ...
    get_data = SimpleHttpOperator(
        task_id='get_op',
        method='POST',
        http_conn_id='insight-gcp-data-pipeline',
        endpoint='download_combustivel',
        data=json.dumps({
            "bucket_name": "data-pipeline-insight-combustiveis-brasil-raw",
    ...
    PYSPARK_JOB = {
        ...
        "pyspark_job": {
            ...
            "args": [
                '--path_input',
                "gs://data-pipeline-insight-combustiveis-brasil-raw/combustiveis-brasil/{{ dag_run.logical_date.strftime('%Y') }}/{{ '01' if dag_run.logical_date.month <= 6 else '02'}}/ca-{{ dag_run.logical_date.strftime('%Y') }}-{{ '01' if dag_run.logical_date.month <= 6 else '02'}}.csv",
                '--path_output',
                "gs://data-pipeline-insight-combustiveis-brasil-curated/combustiveis-brasil/{{ dag_run.logical_date.strftime('%Y') }}/{{ '01' if dag_run.logical_date.month <= 6 else '02'}}/",
                '--file_format', 'parquet',
