...
def start_or_create_spark():
    ...
    spark = (SparkSession
             .builder
             .appName("Processamento de Dados Gasolina Brasil")
...
def write_bigquery(dataframe, bq_dataset, bq_table, gcs_tmp_bucket):
    ...
    dataframe.write         .format("bigquery")         .option("table", "{}.{}".format(bq_dataset, bq_table))         .option("temporaryGcsBucket", gcs_tmp_bucket)         .mode('append')         .save()
...
def main(path_input, path_output, file_format, bq_dataset, bq_table, gcs_tmp_bucket):
    try:
        ...
        df = df.withColumn("numero_rua", col("numero_rua").cast(IntegerType()))             .withColumn("ano", col("ano").cast(StringType()))             .withColumn("semestre", col("semestre").cast(StringType()))             .withColumn('valor_de_venda', regexp_replace('valor_de_venda', ',', '.').cast(FloatType()))             .withColumn('valor_de_compra', regexp_replace('valor_de_compra', ',', '.').cast(FloatType()))
...
if __name__ == '__main__':
    ...
    main(path_input=known_args.path_input,
         ...
         gcs_tmp_bucket="data-pipeline-insight-combustiveis-brasil-pyspark-code"
         )
...
#if __name__ == '__main__':
#    data = main(path_input='gs://data-pipeline-insight-combustiveis-brasil-raw/combustiveis-brasil/2019/02/ca-2019-02.csv',
#                path_output='gs://data-pipeline-insight-combustiveis-brasil-curated/combustiveis-brasil/2019/02/',
#                file_format="parquet",
#                bq_dataset="gasolina_brasil",
#                bq_table="tb_historico_combustivel_brasil",
#                gcs_tmp_bucket="data-pipeline-insight-combustiveis-brasil-pyspark-tmp"
#    )
...
#data.toPandas()
#data.dtypes
#data.groupBy("data").count().show(120)
